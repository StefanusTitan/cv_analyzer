...
CV analysis configuration is read from environment variables. The required values are `DASHSCOPE_API_KEY` and optionally `GITHUB_ACCESS_TOKEN`; the default native DashScope endpoint is `https://dashscope-intl.aliyuncs.com/api/v1` with model `qwen3.7-flash-2026-07-15` and thinking disabled. The API accepts PDF, DOCX, and legacy DOC uploads at `POST /cv/analyze` as multipart fields `job_posting_id` and `files`. It retrieves the job title and HTML description from `${JOB_POSTING_API_URL}/{job_posting_id}`, converts the description to plain text, and returns a concise hiring assessment using the dashboard's simple HTML subset, together with trusted evidence sources and warnings. External evidence citations in the assessment use the full source URLs returned in the `sources` metadata.
